...
log.append = function(file,str,callback){
  ...
  fs.open(log.baseDir+file+'.log', 'a', function(err, fileDescriptor){
...
log.list = function(includeCompressedLogs,callback){
  fs.readdir(log.baseDir, function(err,data){
...
log.compress = function(logId,newFileId,callback){
  ...
  fs.readFile(log.baseDir+sourceFile, 'utf8', function(err,inputString){
    if(!err && inputString){
...
log.decompress = function(fileId,callback){
  ...
  fs.readFile(log.baseDir+fileName, 'utf8', function(err,str){
    if(!err && str){
      // Inflate the data
...
log.truncate = function(logId,callback){
  fs.ftruncate(log.baseDir+logId+'.log', 0, function(err){
    if(!err){
      callback(false);
    } else {
      callback(err);
